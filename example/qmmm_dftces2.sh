...
MDEQUIL="1000000" # no. of equilibration steps
MDAVERAGE="1000000" # no. of averaging steps
...
initialqm=0 #1, when the initial qm has been done.
firstrun=1
cube_coul_QM=(solute.pot.cube) # v_q^A
cube_rho_QM=(solute.rho.cube) # rho^A
cube_QM_rho_hat=(c_rho_oxygen.cube c_rho_hydrogen.cube) # \hat rho^A sigma_b
cube_output_MM=(oxygen.cube hydrogen.cube) # rho_q^B
cube_output_PolarDen=(px.cube py.cube pz.cube) # Polarization density
TIP4Pinvolved=(1 0) # when TIP4P water is involved in the syste, first: 1, second: order of output MM cube for TIP4P oxygen 
...
post_dipole (){
        ...
        $CUBEADD 4 postdipx.cube postdipy.cube postdipz.cube mobile.cube
...
for ((qmmmstep=$QMMMINISTEP; qmmmstep<=$QMMMFINSTEP; qmmmstep++));do
  ...
  sed -i "s/.*fileout.*/fileout = 'solute.rho.cube'/" pp.rho.in
    ...
    if [ $initialqm -eq 0 ]; then
        ...
        $CUBEADD 2 v_saw.cube solute.pot.cube
    ...
    echo "### Running LAMMPS(emxext) for equilibration $qmmmstep QMMM iterations"$'\n'
		nrestart=`echo ${MDEQUIL} | awk '{print int($1/5)}'`
    sed -i "s/nnrestart/${nrestart}/" in.lammps
