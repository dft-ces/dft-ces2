...
int main(int argc, char *argv[]){
 ...
 if(argc==3){
  ...
  totalcharge = atof(argv[2]);
 }
 else{
  printf("Usage: ./cube_jellium cubefile charge\n");
  return 0;
 }
...
// double hello=height/0.52917721; // Angs to bohr
// int pz=hello/base.cellParams[2]*gnz;
// printf("%d \n", pz);  from which grid to set values zero 
// printf("%d \n", gnz); max grid height in grid number
// printf("%f \n", hello);
 double dchd=totalcharge/(base.cellParams[0]*base.cellParams[1]*base.cellParams[2]);
 for(i=0;i<base.pGrid[0]*base.pGrid[1]*base.pGrid[2];i++){
   base.pValue[i] = dchd;
 }
 saveCube(base,"jellium.cube");
