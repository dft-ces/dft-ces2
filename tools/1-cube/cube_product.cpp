...
tag_Models potParser(const char* fname);
void saveCube(tag_Models input, const char* fname);
...
int main(int argc, const char *argv[]){
 ...
 if(argc==3){
  ...
  printf("for cube1*cube2, Usage: ./cube_product cube1 cube2\n");
  return 0;
 }

 tag_Models base1, base2;
 ...
 for(i=0;i<base1.pGrid[0];i++){
   for(j=0;j<base1.pGrid[1];j++){
     for(k=0;k<base1.pGrid[2];k++){
       base1.pValue[k+j*base1.pGrid[2]+i*base1.pGrid[2]*base1.pGrid[1]] *= base2.pValue[k+j*base1.pGrid[2]+i*base1.pGrid[2]*base1.pGrid[1]];
     }
   }
 }

 saveCube(base1,"product.cube");
}//main END

tag_Models potParser(const char* fname){ // LOCPOT parser
 FILE* fp;
