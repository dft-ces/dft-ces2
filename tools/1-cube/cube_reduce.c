...
main(int argc, char *argv[]){
 ...
 if(argc==3){
  ...
  printf("Usage: ./cube_reduce *.cube #Grid-z-dir\n");
  return 0;
 }
 zGrid=atoi(argv[2]);
 ...
 out.cellParams[0] = base.cellParams[0];
 out.cellParams[1] = base.cellParams[1];
 out.cellParams[2] = base.cellParams[2]/base.pGrid[2]*zGrid;

 out.nAtoms = base.nAtoms;
 out.pGrid[0] = base.pGrid[0];
 out.pGrid[1] = base.pGrid[1];
 out.pGrid[2] = zGrid;

 out.atomN = (int *) malloc(sizeof(int)*out.nAtoms);
 out.posX = (double *) malloc(3*sizeof(double)*out.nAtoms);
 out.posY = (double *) malloc(3*sizeof(double)*out.nAtoms);
 out.posZ = (double *) malloc(3*sizeof(double)*out.nAtoms);
 
 for(i=0;i<base.nAtoms;i++){
  out.atomN[i] = base.atomN[i];
  out.posX[i] = base.posX[i];
  out.posY[i] = base.posY[i];
  out.posZ[i] = base.posZ[i];
 }
 
 out.pValue = (double *) malloc(sizeof(double)*out.pGrid[0]*out.pGrid[1]*out.pGrid[2]);

 double tmp=0;

 for(i=0;i<out.pGrid[0];i++){
  for(j=0;j<out.pGrid[1];j++){
   for(k=0;k<out.pGrid[2];k++){
    out.pValue[k+j*out.pGrid[2]+i*out.pGrid[2]*out.pGrid[1]] = base.pValue[k+j*base.pGrid[2]+i*base.pGrid[2]*base.pGrid[1]];
   }
  }
 } 

 saveCube(out,"reduce.cube");
...
void saveCube(tag_Models input, char* fname){
  ...
  fprintf(fp," Cubefile created by QE_merge\n");
