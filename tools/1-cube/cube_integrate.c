...
main(int argc, char *argv[]){
 ...
 if(argc==3){
  ...
  printf("for cube1-cube2, Usage: ./cube_integrate cube1 cube2\n");
  return 0;
 }

 tag_Models base1, base2, out;
 base1=potParser(potFile1); 
 base2=potParser(potFile2);
 out=base1;
 double integrated=0.0;
 for(i=0;i<base1.pGrid[0]*base1.pGrid[1]*base1.pGrid[2];i++){
   out.pValue[i] = base1.pValue[i] * base2.pValue[i];
   integrated += out.pValue[i];
 }
 integrated = integrated * (base1.cellParams[0]/base1.pGrid[0]) * (base1.cellParams[1]/base1.pGrid[1]) * (base1.cellParams[2]/base1.pGrid[2]);
 printf("%lf\n",integrated);

 saveCube(out,"integrated.cube");
