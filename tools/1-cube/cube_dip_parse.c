...
int main(int argc, char *argv[]){
 ...
 if(argc==3){
  ...
  printf("Usage: ./cube_dip_parse cubefile dir(1(x)|2(y)|3(z))\n");
  return 0;
 }

 tag_Models base;
 base=potParser(potFile); 
 tag_Models basei;
 basei=potParser(potFile); 
 int gnx=base.pGrid[0];
 int gny=base.pGrid[1];
 int gnz=base.pGrid[2];

 for(i=0;i<base.pGrid[0]*base.pGrid[1]*base.pGrid[2];i++){
   basei.pValue[i] = 0;
 }
 // LAMMPS convention to QE convention of sign, conversion done here.
 if(dir==1){
	 printf("Calculation along x-direction\n");
	 for(i=0;i<gnx;i++){
		 for(j=0;j<gny;j++){
			 for(k=0;k<gnz;k++){
				basei.pValue[k+j*gnz+((gnx+i-1)%gnx)*gny*gnz]+=base.pValue[k+j*gnz+i*gny*gnz]/(2*base.cellParams[0]/gnx); // [gnx-1]-=dipole/s is physical. but the convention 
				basei.pValue[k+j*gnz+((i+1)%gnx)*gny*gnz]-=base.pValue[k+j*gnz+i*gny*gnz]/(2*base.cellParams[0]/gnx);
			 }
		 }
	 }
 }
 if(dir==2){
	 printf("Calculation along y-direction\n");
	 for(i=0;i<gnx;i++){
		 for(j=0;j<gny;j++){
			 for(k=0;k<gnz;k++){
				basei.pValue[k+((gny+j-1)%gny)*gnz+i*gny*gnz]+=base.pValue[k+j*gnz+i*gny*gnz]/(2*base.cellParams[1]/gny);
				basei.pValue[k+((j+1)%gny)*gnz+i*gny*gnz]-=base.pValue[k+j*gnz+i*gny*gnz]/(2*base.cellParams[1]/gny);
			 }
		 }
	 }
 }
 if(dir==3){
	 printf("Calculation along z-direction\n");
	 for(i=0;i<gnx;i++){
		 for(j=0;j<gny;j++){
			 for(k=0;k<gnz;k++){
				basei.pValue[((gnz+k-1)%gnz)+j*gnz+i*gny*gnz]+=base.pValue[k+j*gnz+i*gny*gnz]/(2*base.cellParams[2]/gnz);
				basei.pValue[(k+1)%gnz+j*gnz+i*gny*gnz]-=base.pValue[k+j*gnz+i*gny*gnz]/(2*base.cellParams[2]/gnz);
			 }
		 }
	 }
...
// int Ngrid = base.pGrid[0]*base.pGrid[1]*base.pGrid[2];
// printf("%e \n", base.pValue[(-1+Ngrid)%Ngrid]);
// printf("%e \n", base.pValue[(Ngrid-1+Ngrid)%Ngrid]);

// saveCube(base,"multiplied.cube");
 saveCube(basei,"postdipole.cube");
